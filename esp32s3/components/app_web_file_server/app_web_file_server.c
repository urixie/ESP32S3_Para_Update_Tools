...
esp_err_t app_web_file_server_start(const char *mount_point)
{
    ...
    ESP_GOTO_ON_ERROR(ret, err_stop, TAG, "register /api/flash-dump/start failed");
    ret = httpd_register_uri_handler(s_server, &flash_dump_status_uri);
    ESP_GOTO_ON_ERROR(ret, err_stop, TAG, "register /api/flash-dump/status failed");
    ret = httpd_register_uri_handler(s_server, &flash_dump_data_uri);
    ESP_GOTO_ON_ERROR(ret, err_stop, TAG, "register /api/flash-dump/data failed");
