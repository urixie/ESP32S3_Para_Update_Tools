...
|    8 | name_offset    | `u16`  |    2 | 名称表偏移（相对 Name Table）     |
...
typedef struct {
    ...
    uint32_t default_value;  /* 0 ~ 4294967295                  */
...
解析端只支持当前 `Payload schema_version = 3`。后续如果调整字段偏移或长度，
需要同步提升 schema_version 并更新 ESP32 端解析代码。
